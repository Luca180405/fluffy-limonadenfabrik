<script>
    import { metatags } from '@roxi/routify'
    
    metatags.title = 'Fluffy einkaufen'
    metatags.description = 'Description coming soon...'
    
</script>


<div class="bg-gray-50">
    <div class="max-w-2xl px-4 mx-auto py-14 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8">
      <h2 class="text-2xl font-extrabold tracking-tight text-gray-900">Bei Fluffy einkaufen</h2>
  
      <div class="grid grid-cols-1 mt-6 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
        <div class="relative group">
          <div class="w-full overflow-hidden bg-gray-200 rounded-md min-h-80 aspect-w-1 aspect-h-1 group-hover:opacity-75 lg:h-80 lg:aspect-none">
            <img src="/img/limo1.png" alt="Fluffy Limo" class="pt-16 pl-3 pr-3">
          </div>
          <div class="flex justify-between mt-4">
            <div>
              <h3 class="text-sm text-gray-700">
                <a href="/produkte/fluffy-getraenkedosen">
                  <span aria-hidden="true" class="absolute inset-0"></span>
                  Fluffy Getränkedosen
                </a>
              </h3>
            </div>
            <p class="text-sm font-medium text-gray-900">0,99€</p>
          </div>
        </div>
        
        <div class="relative group">
            <div class="w-full overflow-hidden bg-gray-200 rounded-md min-h-80 aspect-w-1 aspect-h-1 group-hover:opacity-75 lg:h-80 lg:aspect-none">
              <img src="/img/limo_erzi.png" alt="Erzi Getränkedosen" class="lg:pt-14 md:pt-9 sm:pt-5">
            </div>
            <div class="flex justify-between mt-4">
              <div>
                <h3 class="text-sm text-gray-700">
                  <a href="/produkte/erzi-getraenkedosen">
                    <span aria-hidden="true" class="absolute inset-0"></span>
                    Erzi Getränkedosen
                  </a>
                </h3>
              </div>
              <p class="text-sm font-medium text-gray-900">1,49€</p>
            </div>
          </div>
          
          <div class="relative group">
            <div class="w-full overflow-hidden bg-gray-200 rounded-md min-h-80 aspect-w-1 aspect-h-1 group-hover:opacity-75 lg:h-80 lg:aspect-none">
              <img src="/img/limo6.png" alt="TAU Orange-Limonade" class="pt-12">
            </div>
            <div class="flex justify-between mt-4">
              <div>
                <h3 class="text-sm text-gray-700">
                  <a href="/produkte/tau-orange-limonade">
                    <span aria-hidden="true" class="absolute inset-0"></span>
                    TAU Orange-Limonade
                  </a>
                </h3>
              </div>
              <p class="text-sm font-medium text-gray-900">8,99€</p>
            </div>
          </div>
          
          <div class="relative group">
            <div class="w-full overflow-hidden bg-gray-200 rounded-md min-h-80 aspect-w-1 aspect-h-1 group-hover:opacity-75 lg:h-80 lg:aspect-none">
              <img src="/img/limo_kiste.png" alt="Fluffy Mehrwegkiste" class="pt-12">
            </div>
            <div class="flex justify-between mt-4">
              <div>
                <h3 class="text-sm text-gray-700">
                  <a href="/produkte/fluffy-mehrwegkiste">
                    <span aria-hidden="true" class="absolute inset-0"></span>
                    Fluffy Mehrwegkiste
                  </a>
                </h3>
              </div>
              <p class="text-sm font-medium text-gray-900">7,99€</p>
            </div>
          </div>
          
          <div class="relative group">
            <div class="w-full overflow-hidden bg-gray-200 rounded-md min-h-80 aspect-w-1 aspect-h-1 group-hover:opacity-75 lg:h-80 lg:aspect-none">
              <img src="/img/limo3.png" alt="Fluffy Familienglück" class="pt-12">
            </div>
            <div class="flex justify-between mt-4">
              <div>
                <h3 class="text-sm text-gray-700">
                  <a href="/produkte/fluffy-familienglueck">
                    <span aria-hidden="true" class="absolute inset-0"></span>
                    Fluffy Familienglück
                  </a>
                </h3>
              </div>
              <p class="text-sm font-medium text-gray-900">2,99€</p>
            </div>
          </div>
          
          <div class="relative group">
            <div class="w-full overflow-hidden bg-gray-200 rounded-md min-h-80 aspect-w-1 aspect-h-1 group-hover:opacity-75 lg:h-80 lg:aspect-none">
              <img src="/img/limo5.png" alt="Bionade" class="relative sm:pl-7 md:pl-9 md:pt-1 sm:pt-0 lg:right-6 lg:top-3">
            </div>
            <div class="flex justify-between mt-4">
              <div>
                <h3 class="text-sm text-gray-700">
                  <a href="/produkte/Bionade">
                    <span aria-hidden="true" class="absolute inset-0"></span>
                    Bionade
                  </a>
                </h3>
              </div>
              <p class="text-sm font-medium text-gray-900">1,99€</p>
            </div>
          </div>
          
          <div class="relative group">
            <div class="w-full overflow-hidden bg-gray-200 rounded-md min-h-80 aspect-w-1 aspect-h-1 group-hover:opacity-75 lg:h-80 lg:aspect-none">
              <img src="/img/bitter_lemon.png" alt="Fluffy Bitter Lemon" class="h-full pb-2 pl-16 plact-2">
            </div>
            <div class="flex justify-between mt-4">
              <div>
                <h3 class="text-sm text-gray-700">
                  <a href="/produkte/bitter-lemon">
                    <span aria-hidden="true" class="absolute inset-0"></span>
                    Fluffy Bitter Lemon
                  </a>
                </h3>
              </div>
              <p class="text-sm font-medium text-gray-900">3,99€</p>
            </div>
          </div>
          
          <div class="relative group">
            <div class="w-full overflow-hidden bg-gray-200 rounded-md min-h-80 aspect-w-1 aspect-h-1 group-hover:opacity-75 lg:h-80 lg:aspect-none">
              <img src="/img/limo2.gif" alt="Fluffy Sekt" class="h-full pt-2 pb-2 pl-16">
            </div>
            <div class="flex justify-between mt-4">
              <div>
                <h3 class="text-sm text-gray-700">
                  <a href="/produkte/fluffy-sekt">
                    <span aria-hidden="true" class="absolute inset-0"></span>
                    Fluffy Sekt
                  </a>
                </h3>
              </div>
              <p class="text-sm font-medium text-gray-900">6,99€</p>
            </div>
          </div>
          
      </div>
    </div>
  </div>
